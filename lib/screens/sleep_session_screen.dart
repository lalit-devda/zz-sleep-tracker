...
class _SleepSessionScreenState extends State<SleepSessionScreen> with TickerProviderStateMixin {
  ...
  void _triggerAlarm() {
    ...
    if (kIsWeb) js.context['sleepAudio']?.callMethod('startAlarm');
    setState(() {
  ...
  void _dismissAlarm() {
    if (kIsWeb) js.context['sleepAudio']?.callMethod('stopAlarm');
    setState(() {
  ...
  void _changeSound(String type) {
    if (_selectedSound == type) {
      if (kIsWeb) js.context['sleepAudio']?.callMethod('stop');
      setState(() {
        _selectedSound = 'none';
      });
    } else {
      if (kIsWeb) {
        ...
        js.context['sleepAudio']?.callMethod('setVolume', [_volume]);
      }
      setState(() {
        _selectedSound = type;
      });
  ...
  void _setVolume(double val) {
    if (kIsWeb) js.context['sleepAudio']?.callMethod('setVolume', [val]);
    setState(() {
      _volume = val;
    });
  }
